...
var dataDB;
var objectStore;
var DB_NAME = 'dictionary';
var DB_VERSION = 1;
var STORE_NAME = 'vocab';
var db;

var addButton = document.querySelector('#btngo');
...
request.onupgradeneeded = function(event) {
  ...
  // this DB_NAME/DB_VERSION combo.
	//var objectStore=request.result.createObjectStore([STORE_NAME], {keyPath: 'id', autoIncrement:true});
	var objectStore = db.createObjectStore([STORE_NAME], { keyPath: "id", autoIncrement:true });
	   ...
	   objectStore.createIndex('_rev', '_rev', { unique: false });
        objectStore.createIndex('defID', 'defID', { unique: false });
};
...
function selectOneDefinition(docId) {
    ...
    selectedDocument.defID=docId;
     var transaction = db.transaction([STORE_NAME], 'readwrite');
            var objectStore = transaction.objectStore(STORE_NAME);
            console.log(objectStore);
             console.log(selectedDocument.defID);
                objectStore.put(selectedDocument).onsuccess = function() {
                console.log('Added vocabulary to IndexedDB.');
               console.log(selectedDocument.defID);
...
function deleteOneDefinition(docId) {
 var searchword = $("#searchword").val();
  ...
  let request = objectStore.delete(parseInt(deletedDocument.id));
  $("#select_" + docId).html("<a href=\"javascript:selectOneDefinition('" + docId + "');\">SELECT</a>").show();// show button
